    ...
    for _, row in df.iterrows():
        ...
        if ("repeats" in cur_item) and (cur_item["repeats"] > 0):
            if cur_item["column_name_jp"][0] == "<":
                ...
                assert cur_column_object["sub_seq"] == ""
                del cur_column_object["sub_seq"]
                del cur_column_object["repat_item_handling"]
                cur_column_object["sub_columns_info"] = {
                    "repeats": cur_item["repeats"],
                    "repat_item_handling": cur_item["repat_item_handling"],
                    "sub_columns": []
                }
        ...
        # 繰り返しではない場合
            ...
            # sub_seqは（おそらく）空のはずなので、項目を削除する
            assert cur_item["sub_seq"] == ""
            del cur_item["sub_seq"]
